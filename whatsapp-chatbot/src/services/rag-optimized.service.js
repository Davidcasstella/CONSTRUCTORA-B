...
const RAG_CONFIG = {
  ...
  // Umbrales de calidad - AJUSTADOS para OpenAI embeddings en español
  // OpenAI embeddings producen cosine similarity inherentemente bajo para español
  // (un match casi exacto "quien es david" vs el documento produjo solo 0.21)
  thresholds: {
    high: 0.40,      // Calidad alta - responder con confianza
    medium: 0.25,    // Calidad media - responder con contexto
    low: 0.15,       // Calidad baja - aún puede responder
    escalate: 0.10,  // Solo escalar si realmente no hay NADA relevante
  },
...
function evaluateEscalation(searchResult, query) {
  ...
  // Evaluar escalación - PERMISIVO para español
  // OpenAI embeddings en español producen scores bajos (0.20 = buen match)
